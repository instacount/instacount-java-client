...
public class InstacountClientTest
{
	...
	public void before()
	{
		final InstacountClientParams params = new InstacountClientParams(false);
		...
		if (accessToken != null)
		{
			final InstacountOAuth2BearerRequestInterceptor oauth2 = new InstacountOAuth2BearerRequestInterceptor.Impl(
				accessToken);
			this.client = Instacount.Builder.build(params, oauth2);
		}
		else
		{
			this.client = Instacount.Builder.build(params);
		}
	}
	...
	/**
	 * Required parameters for the Instacount Client.
	 */
	private static class InstacountClientParams extends AbstractInstacountClientParams
	{
		private InstacountClientParams(final boolean readOnly)
		{
			super(readOnly);
		}
